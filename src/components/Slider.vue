<template lang="html">
  <div class="slider-container">
    ...
    <div class="slider-container__info">
      ...
      <h1 v-html="FrontendJson[Projectnumb].Title"></h1>
      <p class="descr" v-html="FrontendJson[Projectnumb].Descr"></p>
    </div>

    <router-link :to="FrontendJson[Projectnumb].Link">
      <div
        class="slider-container__image"
        :style="{
          'background-image':
            'url(' +
            require(`@/assets/${FrontendJson[Projectnumb].Image}`) +
            ')'
        }"
      ></div>
    </router-link>
...
export default {
  ...
  created() {
    const parsedData = JSON.parse(JSON.stringify(this.FrontendJson));
